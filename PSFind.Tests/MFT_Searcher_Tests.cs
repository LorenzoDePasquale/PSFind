...
public class MFT_Searcher_Tests
{
    ...
    public void SearchFile()
    {
        ...
        var results = searcher.Search(s => s == fileName, false)?.ToArray(); // Conversion to array to avoid enumerating twice the IEnumerable result (thus performing the search twice!)
    ...
    public void SearchDirectory()
    {
        ...
        char drive = Environment.CurrentDirectory[0];
        using var searcher = new MFT_Searcher(drive);
        var results = searcher.Search(s => s == directoryName, true)?.ToArray(); // Conversion to array to avoid enumerating twice the IEnumerable result (thus performing the search twice!)
